...
function viewAllRoles() {
  db.promise()
    .query(
      "SELECT role.title, role.id, department_id, role.salary FROM role INNER JOIN department ON role.department_id = department_id"
    )
...
function viewAllEmployees() {
  db.promise()
    .query(
      "SELECT employee.id, employee.first_name, employee.last_name, employee.role_id, department_id, role.salary, employee.manager_id FROM employee INNER JOIN role ON employee.role_id = role.id"
    )
...
function updateRole() {
  db.query(
    'SELECT CONCAT(employee.first_name, " ", employee.last_name) AS full_name, employee.id AS employee_id, role.* FROM employee INNER JOIN role ON employee.role_id = role.id',
    function (err, results) {
      ...
      inquirer
        ...
        .then((answer) => {
          ...
          db.query(
            `UPDATE employee SET role_id = ${updatedRole} WHERE id = ${updateId};`,
            function (err, results) {
              if (err) {
                throw err;
              }
